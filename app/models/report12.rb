# Report on Computed field and Process
...
class Report12 < Report
  ...
  def initialize (settings = nil)
    ...
    possible_values = CustomValue.joins('INNER JOIN issues ON issues.id = custom_values.customized_id').where(:custom_field_id => settings.computed_field).select(:value).uniq.pluck(:value).sort
    @data = []
    @data += possible_values.map { |pv| {
        :name => pv,
        :data => CustomValue.where(:custom_field_id => settings.computed_field, :value => pv)
